...
module.exports = class {
    ...
    async run(member) {
        ...
        if(inviterDataX.inviter) {
            ...
            this.client.channels.cache.get("908075172785979400").send(`<@${inviterDataX.inviter}> tarafından davet edilen \`${member.user.tag}\` sunucudan çıktı. (<@${inviterDataX.inviter}> \`${regular + bonus + fakecount}\` toplam davete sahip oldu.)`)
        } else {
            this.client.channels.cache.get("908075172785979400").send(`\`${member.user.tag}\` sunucudan çıkış yaptı. Veritabanında kullanıcıyı davet eden üye bulunamadı!`)
        }
    }
}