...
//
    async run(member) {
        ...
        if(arr.length > 0) {
          ...
          if(InviteUser.id == member.id) return this.client.channels.cache.get("908075172785979401").send("🎉 1962'ye hoş geldin <@!" + member + "> !\n\nHesabın " + moment(member.user.createdTimestamp).format("LLL") + " tarihinde (" + moment(member.user.createdTimestamp).fromNow() + ") oluşturulmuş. \n\nSunucu kurallarımız <#908075173243138065> kanalında belirtilmiştir. Unutma sunucu içerisinde ki ceza işlemlerin kuralları okuduğunu varsayarak gerçekleştirilecek.\n\nSeninle beraber " + member.guild.members.cache.size + " kişi olduk! Sol tarafta bulunan **✩ Confirmation** odalarından birine girerek kayıt işlemini gerçekleştirebilirsin."), this.client.channels.cache.get("908075172785979400").send(`${member} sunucuya katıldı, kullanıcı kendi kendini davet ettiği için işlem yapılmadı.`)
          ...
          this.client.channels.cache.get("908075172785979401").send("🎉 1962'ye hoş geldin <@!" + member + "> !\n\nHesabın " + moment(member.user.createdTimestamp).format("LLL") + " tarihinde (" + moment(member.user.createdTimestamp).fromNow() + ") oluşturulmuş. \n\nSunucu kurallarımız <#908075173243138065> kanalında belirtilmiştir. Unutma sunucu içerisinde ki ceza işlemlerin kuralları okuduğunu varsayarak gerçekleştirilecek.\n\n<@" + InviteUser.id + "> " + total + ". davetini gerçekleştirerek sunucumuzun " + member.guild.members.cache.size + ". üyesi olmanı sağladı! İyi eğlenceler 🎉🎉🎉")
          this.client.channels.cache.get("908075172785979400").send(`${member} kullanıcısı ${InviteUser} tarafından sunucuya davet edildi. (${InviteUser} \`${regular + bonus + fakes}.\` davetine ulaştı.)`)
        } else {
          this.client.channels.cache.get("908075172785979401").send("🎉 1962'ye hoş geldin <@!" + member + "> !\n\nHesabın " + moment(member.user.createdTimestamp).format("LLL") + " tarihinde (" + moment(member.user.createdTimestamp).fromNow() + ") oluşturulmuş. \n\nSunucu kurallarımız <#908075173243138065> kanalında belirtilmiştir. Unutma sunucu içerisinde ki ceza işlemlerin kuralları okuduğunu varsayarak gerçekleştirilecek.\n\nSeninle beraber " + member.guild.members.cache.size + " kişi olduk! Sol tarafta bulunan **✩ Confirmation** odalarından birine girerek kayıt işlemini gerçekleştirebilirsin."), this.client.channels.cache.get("908075172785979400").send(`${member} sunucuya katıldı fakat kimin davet ettiği bulunamadı.`)
        }
