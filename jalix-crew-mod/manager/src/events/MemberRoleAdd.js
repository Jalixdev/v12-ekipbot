...
module.exports = async(member, role) => {
  ...
  if(!GeneralData) {let newGeneralDatabase = new GeneralDatabase({guildID: member.guild.id, userID: member.id, rollogtotal: 1, rollog: [{role: `Rol: ${role} Yetkili: ${entry.executor}\nTarih: ${moment(Date.now()).locale("TR").format("LLL")}`}]}).save();} else{
  GeneralData.rollog.push({role: `Rol: ${role} Yetkili: ${entry.executor}\nTarih: ${moment(Date.now()).locale("TR").format("LLL")}`}); 
