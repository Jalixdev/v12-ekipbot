...
client.emojiSayi = function(sayi) {
  ...
  client.punish = (kisiID, tur, sebep) => {
   let vegas = client.guilds.cache.get(cfg.Server.GuildID).members.cache.get(kisiID);
   if (!vegas) return;
  ...
  client.setRoles = (memberID, rolID) => {
   let victim = client.guilds.cache.get(cfg.Server.GuildID).members.cache.get(memberID);
   if(victim.roles.cache.has(cfg.Roles.BoosterRolü)) {
