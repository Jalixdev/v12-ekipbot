...
module.exports = async (guild, member) => {
  ...
  let entry = await guild.fetchAuditLogs({ limit: 1, type: 'MEMBER_BAN_REMOVE' }).then(x => x.entries.first());
  ...
  await guild.members.ban(member.id, {reason: "Unban Guard"})
  client.logSend(`${member} üyesinin yasaklaması kaldırıldı ve üyeyi tekrar banlayıp, banı açan kişiyi banladım.\n───────────────\nÜye: ${member} (\`${member.tag.replace("`", "")}\` - \`${member.id}\`)\nYetkili: ${entry.executor} (\`${entry.executor.tag.replace("`", "")}\` - \`${entry.executor.id}\`)`)
