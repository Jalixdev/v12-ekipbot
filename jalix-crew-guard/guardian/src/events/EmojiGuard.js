...
module.exports = async emoji => {
  ...
  let entry = await emoji.guild.fetchAuditLogs({ limit: 1, type: 'EMOJI_DELETE' }).then(x => x.entries.first());
  ...
  client.logSend(`${entry.executor} üyesi emoji sildi ve emojiyi geri yükleyip, silen kişiyi banladım.\n───────────────\nEmoji: \`${emoji.name}\` (\`${emoji.id}\`)\nYetkili: ${entry.executor} (\`${entry.executor.tag.replace("`", "")}\` - \`${entry.executor.id}\`)`)
